...
class GTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = Encoder(config.dim_model, config.GT_num_head, config.GT_hidden, config.dropout)
        self.encoders = nn.ModuleList([
            copy.deepcopy(self.encoder)
            for _ in range(config.GT_num_encoder)])
        self.fc1 = nn.Linear(config.dim_model, config.dim_model)
...
class Positional_Encoding(nn.Module):
    def __init__(self, embed, pad_size, dropout, device):
        ...
        self.device = device
        self.pe = torch.tensor([[pos / (10000.0 ** (i // 2 * 2.0 / embed)) for i in range(embed)] for pos in range(2*pad_size)])
